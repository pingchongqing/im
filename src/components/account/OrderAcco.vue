...
<div class="goodslist-03" style=" display:block;" v-if="elem.Show&&elem.DialogShow">
    ...
    <div class="top-header border-bottom fixed-top">
         <div class="top-back "><a  @click="elem.Show=elem.DialogShow=false">返回</a></div>
    ...
    <div >
        ...
        <div class="fixed-button border-top fixed-bottom bg-fff p-30" >
             <div class="redbutton"><a @click="dialogButtonShow(index)" >确定</a></div>
        </div>
              ...
              <div class="fixed-button3 border-top " data-style="fixed-bottom" >
                <div class="redbutton">
                  <span>合计：<em>￥{{totoalAmount}}</em></span>
    ...
    methods: {
      ...
      selectgoodsdetail: function () {
        ...
        if (self.goods_source_type == 0) {
          ...
          if (self.$route.params.gameType == 2) {
            var pcDuanKou = self.$CONSTANTS.APIMainGoods + 'mainMobileGoods/getMainGoodsDetail';
            var typeData = self.$http.post(pcDuanKou, {goodsId: self.goodsId});
          } else if (self.$route.params.gameType == 1) {
            var pcDuanKou = self.$CONSTANTS.APIMainGoods + 'mainGoodsOrderConfig/getAccountOrderConfig';
            var typeData = self.$http.get(pcDuanKou, {params:{BizOfferId: self.goodsId}});
            self.$http.get(self.$CONSTANTS.APIMainGoods + 'mainGoods/selectpcgoodsdetail', {params:{BizOfferId: self.goodsId}}).then(
              res => {
      ...
      GetSafeBuyConfig: function (gameId, goodsType, price) {
        ...
        self.$http.post(self.$CONSTANTS.APISafebuy + 'SafeBuyConfigService/GetSafeBuyConfig', {
          ...
          if (res.body.success == true) {
            ...
            var pice = parseInt(price * res.body.safeBuyConfig.pricePercentBuyer * 10000) / 10000;//计算安心买费用->下面的判断是用来做安心买费用的限制
      ...
      subBotton: function () {
        ...
        self.$http.post(self.$CONSTANTS.APILogin + "account/queryCurrentUserInfo").then(function (res) {
          if (res.body.success == true) {
            if (self.goods_source_type == 0) {
              self.create(self.buyTel, self.buyQ, self.goodsId, self.goodsType, self.num);
            } else {
              if (self.$route.params.gameType == 2) {
      ...
      create: function (buyerPhone, buyerQq, goodsId, goodsType, num) {
        ...
        self.$http.post(self.$CONSTANTS.APIOrder + "order/account/create", postData).then(function (res) {
          if (res.body.success == true) {
            // sa.track("order",{
            //   goodsWeight:self.$route.params.qz,
...
//            location.href = "https://mypay.5173.com/PaymentWap/PayExtex?" + json_result.BizResult.Data.payUrl;
      ...
      maoMyPay: function (orderId) {
        ...
        self.$http.get(self.$CONSTANTS.APIOrder + "payment/mypay", {
          params: {
            orderId: orderId,
            returnUrl: location.protocol + "//" + window.location.host + '/vue/buyerCenter/buyerOrder/mobgoods/' + orderId + '?from=payed'
          }
        }).then(function (res) {
          if (res.body.success == true) {
