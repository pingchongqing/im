...
<div class="upload">
    ...
    <div v-if="!list ||list.length <= 0">
      ...
      <!--   <div class="border-bottom upLoad-img-box bg-fff text-center">
            <label class="color-m1 border f30 text-center">
           <input type="file" name="imageFile" class="upload" @change="fileChange" :accept="accepts" :multiple="multiple">
           点击选择图片
           </label>
            <p class="f28 color-666">单次最多可选18张</p>
        </div> -->
        ...
        <p class="px-30 py-20 f24 color-666">注：单次最多可选18张</p>
        ...
        <!--上传图片以后的样式-->
   <!--  <div class="bg-fff border-bottom upload-img2 clearfix" v-if="list && list.length > 0">
      <div class="progress" v-if="percent > 0">
            <span :style="{width:percent+'%'}"></span>
            <em class="pa">{{percent}}%</em>
        </div>
        <div class="fl" v-for="(item,index) in list">
            <a href="#"><img class="game-img" :src="item.sImgUrl" alt=""></a>
            <div class="del-mask" >
                <img @click="deleteImg(index)" src="../../../assets/images/Icon/bitch-b.png" alt="" class="close-img1">
            </div>
        </div>
        <div class="fl">
            <label >
    ...
    <div class="upload-img-list bg-fff px-30 pt-30 clearfix" v-if="list && list.length > 0">
        <div class="progress" v-if="percent > 0">
            <span :style="{width:percent+'%'}"></span>
            <em class="pa">{{percent}}%</em>
        </div>

        <div class="fl mb-30 img-list-item" v-for="(item,index) in list">
            <img :src="item.sImgUrl" alt="">
            <i class="del-img" @click="deleteImg(index)"></i>
        </div>
...
export default {
    ...
    created(){
        ...
        this.$on('getImg',(data)=>{
            if (this.list && this.list.length < 18) {
                this.list.push(data);
            };
        })
    },
    methods:{
        ...
        uploadFiles(){
            ...
            vm.$parent.isUpload = false;
            vm.$parent.uploadImgCount = this.files.length;
            let j = 1 ;
                ...
                xhr.onreadystatechange = e=> {
                    if (xhr.readyState == 4) {
                        if (xhr.status == 200) {
                            ...
                            //获取上传之后的数据
                            if (data.success && this.list.length < 18) {
                              if(this.success != undefined){
                                    this.success(data)
                             }
                            };
