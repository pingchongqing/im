<template>
  ...
  <!---------------------------- 角色信息 ---------------------------->
  <div id="coins" class="js-msg bg-fff clearfix" style="margin-bottom: .3rem">


    <ul class="bg-fff">
      <li class="w-100 border-bottom clearfix lh110 px-30 f30 pr" v-if="IsjueseName">
        <span class="f30">收货角色</span>
        <input class="coin-input lh110 pa" style="width: 57%" type="text" name="" value="" id="username"
               v-model="receiver" @click="showCoins()"
               placeholder="请输入接收装备的角色名"/>
        <i class="pa" v-if="isTrue" id="divselect" maxlength="50" @click="show()"><img src="~images/coins/coinuser.png"/></i>
      </li>

      <li class="w-100 border-bottom clearfix lh110 px-30 f30 pr" v-if="IsZhanghoa">
        <span class="f30">游戏账号</span>
        <input class="coin-input lh110 pa" type="tel"  v-model="way_zhanghao" placeholder="请填写你的游戏账号"/>
      </li>

      <li class="w-100 border-bottom clearfix lh110 px-30 f30 pr" v-if="Isshouhuo">
        <span class="f30">角色等级</span>
        <input class="coin-input lh110 pa" type="tel" maxlength="4" v-model="way_dengji" placeholder="请填写收货角色等级"/>
      </li>

      <li class="w-100 border-bottom clearfix lh110 px-30 f30 pr" v-if="Isjuese">
        <span class="f30">角色数字ID</span>
        <input class="coin-input lh110 pa" type="text" maxlength="20" v-model="way_juese" placeholder="请填写角色数字ID"/>
      </li>


      <li v-if="coin_ico" class=" select-qufu clearfix f30 pl-30" @click="trade_qufu">
        <span class="f30">交易区服</span>
      ...
      <li class=" select-mode clearfix f30 pl-30" @click="trade_way" v-if="fangshis">
        <span class="f30">交易方式</span>
        <input class="coin-input lh110 pa" disabled="" type="text" name="" v-model="way_input">
        <span id="toRight" class="to-right"></span>
      </li>

      <li class=" select-mode clearfix f30 pl-30" @click="zy_way" v-if="zhenyings">
        <span class="f30">阵营</span>
        <input class="coin-input lh110 pa" disabled="" type="text" name="" v-model="zy_input">
        <span class="to-right"></span>
      </li>

      <li class="w-100 border-bottom clearfix lh110 px-30 f30 pr">
        <span class="f30">我的手机号</span>
        <input class="coin-input lh110 pa" type="tel" name="" id="phoneFrom" v-model="phoneName"
               placeholder="请输入手机号"/>
      </li>
      <li class="w-100 border-bottom clearfix lh110 px-30 f30 pr">
        <span class="f30">我的QQ号</span>
        <input class="coin-input lh110 pa" type="tel" name="" id="QQFrom" v-model="qqName"
               placeholder="请输入QQ号"/>
      </li>
    </ul>
    <div id="coinsbox">
      <drop-down :xianshi="xianshi" @change="xianshi = !xianshi" v-on:ee="incrementTotal" v-on:ff="mobileNumberTotal" :eqshade="'eqshade'"
                 :role-list="roleList"
...
//          isTrue:this.url == ''?false:true
          ...
          this.$http.get(
            ...
            if(res.body.responseStatus.code == "00"){
              if(res.body.result){
                ...
                this.IsZhanghoa = str.IsSupportGameAccount;
                if(str.IsSupportRole){
                  this.IsjueseName  = true;
                }else{
                  this.IsjueseName = false
                }
  ...
  //              this.fu_input = str.GameServerName;
                ...
                if(str.BuyMode.Key){
                  ...
                  this.wayActive = parseInt(str.BuyMode.Value)
                }
                const attr = [];
...
//            console.log("-------------str-----------",str)
    ...
    mounted()
    {
      ...
      this.$emit('formzhanghao',this.way_zhanghao)
...
//          }
      ...
      way_zhanghao(val,oldVal){
        var patrn = /^[a-zA-Z0-9]+$/;
        if (patrn.test(val)) {
          this.way_zhanghao = this.way_zhanghao
        } else {
          if(this.way_zhanghao===""){
            this.way_zhanghao = this.way_zhanghao
          }else{
            this.way_zhanghao = oldVal
          }
